const SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vR5_ZuCTFWUvfyCNla7wmkg9O0BcM6IOW702XkVpOSbD3dc_4I1Hp1-hSaCLiW9u62wcYDSs9UE2faM/pub?output=csv';
        ...
        function initializeFilters() {
            ...
            const years = [...new Set(salesData.map(x => x['Sales year']))].sort(); // Ensure years are sorted
            ...
            const people = [...new Set(salesData.map(x => x['Sales person name']))].sort(); // Ensure people are sorted
        ...
        function updateMonthlySalesTargetChart() {
            ...
            // --- DEBUGGING LOGS ---
            console.log("--- Monthly Sales Target Chart Data ---");
            console.log("Filtered Data:", filteredData);
            console.log("Monthly Actual Sales (actuals array):", actuals);
            console.log("Monthly Targets (targets array):", targets);
            // --- END DEBUGGING LOGS ---
            ...
            console.log("Calculated Percentages (percent array):", percent); // <--- Critical for debugging
                ...
                monthlyTargetChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        scales: {
                            x: {
                                min: 0,
                                max: 150, // This defines the maximum extent of the bar.
                                ticks: {
                                    callback: v => `${v}%`
                                },
                                title: {
        ...
        function updateCustomYearSales() {
            ...
            // Filter data for the custom financial year (Feb of selectedYear to Jan of selectedYear + 1)
            const customYearData = salesData.filter(d => {
                const year = parseInt(d['Sales year']);
                const month = parseInt(d['Month']);
                return (
                    (year === customYear && month >= 2) || // Feb to Dec of selectedYear
                    (year === customYear + 1 && month === 1) // Jan of selectedYear + 1
                );
            });
